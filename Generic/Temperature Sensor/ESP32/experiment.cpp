...
/**
   ROUTE - "/api/light.json"
   @param void
   @return void
*/
void handleRoute_light() {
  if (checkCookieAuthed()) {
    ...
    // Read sensor
    r = readSensorInt("light");  // Read sensor
    sprintf (r1, "%i", r);          // Convert to Char Array
  
    // Build content
    strcpy(msg, "{result:{\"ROUTE\":\"light\",\"status\":\"OK\", \"value\":\"");
    strcat(msg, r1);
    strcat(msg, "\"}}");
...
void handleRoute_lightHTML() {
  if (checkCookieAuthed()) {
    ...
    // Read sensor
    r = readSensor("light");  // Read sensor
    sprintf (r1, "%i", r);    // Convert to Char Array
...
/**
   ...
   @return char
*/
int readSensorInt (String sensor) {
